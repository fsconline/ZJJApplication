...
//		colorMap.put("def", "#bbd5564f");
    ...
    public static Bitmap toRoundBitmap(Bitmap bitmap) {
        ...
        if (width <= height) {
            ...
            /*
             * 如果宽度大于高度， 高度为半径，绘制圆的位置需要扣除比左右多余的控件 宽度设置为和高度一致
			 */
            roundPx = height / 2;
        ...
        canvas.drawARGB(0, 0, 0, 0); // 填充整个Canvas,相当于设置透明的背景
...
//			first = name.substring(name.length() - 1, name.length());
//			sec = name.substring(name.length() - 1);
//		} else {
//			first = name;
//			sec = "";
//		}
//		String color = colorMap.get(PinYinUtil.converterFromPinYinFirstLetter(first));
...
//		return bitmap;
        ...
        if (!TextUtils.isEmpty(name) && name.length() >= 2) {
            userName = name.substring(name.length() - 2, name.length() - 1);
            first = name.substring(name.length() - 1, name.length());
            sec = name.substring(name.length() - 1);
        } else {
            first = name;
        ...
        String color = (String) colorMap.get(PinYinUtil.converterFromPinYinFirstLetter(first));
        if (TextUtils.isEmpty(color)) {
            color = (String) colorMap.get("def");
        }
        ...
        FontMetricsInt fontMetrics = paint.getFontMetricsInt();
        int baseline = (int) (rectF.top + (rectF.bottom - rectF.top - (float) fontMetrics.bottom + (float) fontMetrics.top) / 2.0F - (float) fontMetrics.top);
        paint.setTextAlign(Paint.Align.CENTER);
//		canvas.drawText(first + sec, (float)(width / 2), (float)baseline, paint);
        canvas.drawText(userName + sec, (float) (width / 2), (float) baseline, paint);
        return bitmap;
    }
...
//			first = name.substring(name.length() - 2, name.length() - 1);
        ...
        String color = (String) colorMap.get(PinYinUtil.converterFromPinYinFirstLetter(String.valueOf(first)));
        if (TextUtils.isEmpty(color)) {
            color = (String) colorMap.get("def");
        }
